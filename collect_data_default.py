...
def proc_src(input_folder, output_folder, out_rot_folder, num_sets, num_views):
    ...
    for i in range(num_sets):
        ...
        rgb_max = np.random.uniform(0.9, 1.1) * np.max(rgbs)
        rgbs /= rgb_max
        ...
        for j in range(num_views):
            ...
            # Process RGB
            out_rgb = os.path.join(output_folder, f'{i:03d}_{j:03d}_image.jpg')
            ...
            env_map /= rgb_max
            out_path = os.path.join(output_folder, f'{i:03d}_{j:03d}_light.hdr')
            cv2.imwrite(out_path, env_map)

            env_path = os.path.join(input_folder, 'envmap', f'{filename}_lightcam.hdr')
            env_map = cv2.imread(env_path, cv2.IMREAD_UNCHANGED)
            env_map /= rgb_max
            out_path = os.path.join(output_folder, f'{i:03d}_{j:03d}_lightcam.hdr')
            cv2.imwrite(out_path, env_map)
            ...
            out_mask = os.path.join(output_folder, f'{i:03d}_{j:03d}_mask.jpg')
            cv2.imwrite(out_mask, mask)

            # Process depth
            depth_path = os.path.join(input_folder, 'depth', f'{filename}_0001.exr')
            dst = os.path.join(output_folder, f'{i:03d}_{j:03d}_depth.exr')
            shutil.copy2(depth_path, dst)

            index += 1

        self_rot_idx = num_sets * num_views + i
        filename = f'{self_rot_idx:03d}'
    ...
    # Get set params
    intrinsics = np.array(intrinsics[index:index+num_sets]).reshape([args.num_sets, 3, 3])
    extrinsics = np.array(extrinsics[index:index+num_sets]).reshape([args.num_sets, 3, 4])

    self_rot_shapes = np.array(self_rot_shapes).reshape([args.num_sets, 2])
...
if __name__ == '__main__':
    ...
    for folder in folders:
        ...
        proc_src(folder, out_folder, out_rot_folder, args.num_sets, args.num_views)

        out_folder = os.path.join(args.out_dir, os.path.split(folder[:-1])[-1], 'target_image')
        s = args.num_views*args.num_sets + args.num_sets
        e = args.num_views*args.num_sets + args.num_sets + args.num_tgts
