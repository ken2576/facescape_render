...
from pose_utils import gen_random_poses, gen_random_frontal_pose
import numpy as np
import bpy
import utils
...
def parse_args(argv):
    ...
    parser.add_argument('--env_path', type=str, metavar='PATH',
                        default=None,
                        help='path to the environment map')
    parser.add_argument('--out_dir', type=str, metavar='PATH',
                        help='path to the output directory')
    parser.add_argument('--num_views', type=int, default=80,
                        help='total camera count')
...
if __name__ == '__main__':
    ...
    fpose, fradius = gen_random_frontal_pose(
                                    args.rmin,
                                    args.rmax,
                                    1)
    rposes, rradii = gen_random_poses(args.rmin,
                                    args.rmax,
                                    args.theta_max,
                                    args.phi_max,
                                    args.num_views-1)
    poses = np.concatenate([fpose, rposes], 0)
    radii = np.concatenate([fradius, rradii], 0)
    ...
    elif args.env_folder is not None:
        env_paths = glob.glob(os.path.join(args.env_folder, '*.hdr'))
        env_path = np.random.choice(env_paths)
        load_envmap(env_path)

    cam_extrinsics = np.zeros((len(poses), 4, 4))
    cam_intrinsics = np.zeros((len(poses), 3, 3))

    rendered_env = []
    for idx, (pose, radius) in enumerate(zip(poses, radii)):
        # Set up camera
        ...
        cam_extrinsics[idx] = np.asarray(utils.listify_matrix(temp))
        cam_intrinsics[idx] = K

        # Render scene
        utils.render(out_dir, f'{idx:03d}',
                    depth_file_output,
                    normal_file_output,
                    mask_file_output,
